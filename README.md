...
하지만 페어 간 지식의 갭이  상황에서 상대적으로 지식이 부족한 사람이 빠르게 지식의 수준을 끌어올리지 않는 이상 이 순기능의 의미는 퇴색될 수밖에 없다.
사실상 `강의` 혹은 `과외`로 전락할 수 있다.큰

나는 R을 활용한 **코딩**만 1년 정도 해본 비전공자라 개발자가 흔히 말하는 **프로그래밍**은 처음 이었다.
...
그렇다면 `각개 프로그래 + 페어와의 토론` 형식으로 진행하는 것도 괜찮지 않을까.
